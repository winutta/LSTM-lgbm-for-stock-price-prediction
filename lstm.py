...
def get_data(seq_len,gap):
    ...
    df = data.DataReader('TGT', 'quandl', start, end)
    ...
    AO_target = np.array(AO_target).reshape(-1,1)

    AO = np.array(AO).reshape(-1,1)
    AH = np.array(AH).reshape(-1,1)
    AL = np.array(AL).reshape(-1,1)
    AC = np.array(AC).reshape(-1,1)
    AV = np.array(AV).reshape(-1,1)
    
    AO = StandardScaler().fit_transform(AO)
    AH = StandardScaler().fit_transform(AH)
    AL = StandardScaler().fit_transform(AL)
    AC = StandardScaler().fit_transform(AC)
    AV = StandardScaler().fit_transform(AV)

    AO_target = AO_target.tolist()
    AO = AO.tolist()
    AH = AH.tolist()
    AL = AL.tolist()
    AC = AC.tolist()
    ...
    ender = np.reshape(ender, (ender.shape[0], ender.shape[1]))  
...
def normalise_windows(window_data,a = True):
    normalised_data = []
    multipliers = []
    if a:
        ...
        return normalised_data, multipliers
